...
static inline short vmem_get_word(VirtualMem* vm,unsigned short address) {
...
void vmachine_run(VirtualMachine* vm) {
	...
	for (;;) {
		...
		r1 = (instruction >> 6) & 0x1f;
		r2 = (instruction >> 11) & 0x1f;
		imm = instruction >> 16;
		isaluop = !(cat == 3);
		switch (cat) {
		    case 0: vm->RegFile[imm & 0x1f] = run_alu_op(op, vm->RegFile[r1], vm->RegFile[r2], vm->carry_flag); break;
